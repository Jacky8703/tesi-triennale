...
- la ricerca degli strumenti (servizi) avviene all'interno dell'ambiente _cloud_ AWS, per motivi di compatibilità con l'infrastruttura aziendale;
...
- l'alt deve seguire le _best practice_ #footnote[https://www.accessibilitychecker.org/guides/alt-text/#anchor3] (conciso, descrittivo e rilevante);
...
- deve essere definito un _prompt_ specifico per tipo di attributo da generare, in modo da essere il più specifici possibile e migliorare i risultati;
...
- top-P basso → rimuove i _token_ con probabilità bassa → solo i _token_ con alta probabilità vengono considerati (top P% della distribuzione, _token_ le cui probabilità sommate raggiungono almeno P%) → intuisce meno e genera risposte probabilmente più accurate, usando però lessico più comune;
...
Ovviamente i _prompt_ sono stati scritti in inglese #footnote[https://kubie.medium.com/stop-prompting-in-your-native-language-if-its-not-english-here-s-why-it-s-costing-you-9a927a2e408d]. \
\
#pad(left: -1in, right: -1in)[
  ...
  ) <prompt-models-bedrock>
]
#pagebreak()
Per quanto riguarda *Data Automation*, essendo la sua funzionalità quella di estrarre informazioni, non si basa su _prompt_ o parametri come i modelli offerti da Bedrock, ma su un meccanismo di _standard output_ e _custom output_: \
\
- *_standard output_* #footnote[https://docs.aws.amazon.com/bedrock/latest/userguide/bda-standard-output.html] è l'_output_ predefinito che viene estratto dai contenuti (documenti, immagini, video e audio) ed è personalizzabile solamente nei limiti offerti dal servizio; ad esempio per le immagini è possibile avere in _output_ un riassunto e una lista di etichette riconosciute, non generate ma presenti all'interno di _IAB taxonomy_ #footnote[https://github.com/InteractiveAdvertisingBureau/Taxonomies/blob/develop/Content%20Taxonomies/Content%20Taxonomy%203.1.tsv]\; \
\
- *_custom output_* #footnote[https://docs.aws.amazon.com/bedrock/latest/userguide/bda-custom-output-idp.html] è l'output personalizzabile dall'utente tramite una struttura contenente le istruzioni su ciò che deve essere estratto dal contenuto (si possono pensare come _mini-prompt_ per ogni attributo da estrarre); questa struttura è chiamata *_blueprint_*, e permette di stabilire lo schema di informazioni ricavate. 
\
