...
- l'alt deve seguire alcune _best practice_ #footnote[#box[https://www.accessibilitychecker.org/guides/alt-text/#anchor3 (ultima visita 05/07/2025)]] (conciso, descrittivo e rilevante);
- la lista di _keyword_ deve essere di un numero limitato (10) ed è stato deciso di dividerle per categoria (6 sugli oggetti, 1 sullo stile, 1 sull'inquadratura, 1 sul colore principale, 1 sull'atmosfera);
...
\
Altri parametri che possono modificare la generazione dei modelli, in particolare la precisione della risposta, sono la temperatura, il _top-P_ e il _top-K_:
...
- _top-P_ basso → rimuove i _token_ con probabilità bassa → solo i _token_ con alta probabilità vengono considerati (top P% della distribuzione, _token_ le cui probabilità sommate raggiungono almeno P%);
- _top-K_ → stesso concetto di _top-P_, solo che nella scelta del _token_ successivo tiene in considerazione i K _token_ con probabilità maggiore.
\
Durante la valutazione ho utilizzato i parametri di temperatura e _top-P_, senza parametro _top-K_ per due motivi: 
+ la funzione di _top-P_ e _top-K_ è la stessa, gestita in modo diverso, quindi usarli entrambi non porta a cambiamenti significativi nella generazione;
+ l'_API_ utilizzata per invocare i modelli (_Converse_ #footnote[https://docs.aws.amazon.com/bedrock/latest/userguide/conversation-inference-call.html (ultima visita 05/07/2025)]) non possiede _top-K_ all'interno del set base di parametri (_inferenceConfig_); per poterlo aggiungere andrebbe specificato nel campo "_AdditionalModelRequestFields_" ma i modelli utilizzano nominativi diversi per questo parametro (Claude "top_k", Nova "topK") e non tutti lo possiedono, quindi viene meno l'utilità di usare un'interfaccia comune per l'_API_, se deve essere modificata a seconda dei modelli utilizzati.
\
